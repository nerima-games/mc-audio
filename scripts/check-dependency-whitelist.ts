 ...
 *
 * ---------------------------------------------------------------------------
 * For mc-audio the allowed set is EMPTY, and that is not an oversight.
 * mc-audio is a tier-1 stable library (plan.md §2.2). It is a *sink*: gameplay
 * and UI push cue requests into it and subscribe to its caption stream, and it
 * never reaches back. In particular mc-audio must not import mc-sim to ask
 * "where is the player, so I can pan this sound" — the caller passes the pose
 * in. That one-way rule is what keeps audio testable in Node with no DOM.
 * Its only org dependency is mc-kernel, which needs no entry (rule 4).
 * ---------------------------------------------------------------------------
 */
export const REPOSITORY_POLICY = {
  thisPackage: '@nerima-games/mc-audio',

  /**
   * The full 16-repository roster of plan.md §2.1.
   *
   * `@nerima-games/mc-kernel` is absent from every row on purpose: it is
   * universally importable, and `checkPolicyConfiguration` rejects any row that
   * names it explicitly.
   *
   * `@nerima-games/mc-playground-kit` is absent from the rows of `mx-gameplay`
   * and `mx-redstone` even though plan.md §3.11/§3.12 list it for them: it is
   * theirs as a devDependency only (rule 6), a devDependency creates no runtime
   * edge, and therefore no graph edge. Its own row is present because the kit
   * itself has real runtime dependencies.
   */
  dependencyGraph: new Map<string, ReadonlySet<string>>([
    // --- Tier 1: stable libraries (plan.md §2.2). Mutually independent, so
    // --- they can be built in parallel. Every row here is empty because their
    // --- only dependency is kernel.
    ...
    // --- Tier 2: foundations. State and services — the NOUNS.
    ...
    // --- Tier 3: experience modules. Rules and UI — the VERBS. There is
    // --- deliberately no edge between any two of these rows (plan.md §2.3-1):
    // --- they talk only through tier-2 services.
    [
    ...
    // --- Tier 4: composition. Layer merging and the stage total order, nothing else.
    [
    ...
    // --- Outside the tiers: the dev workspace of plan.md §6 Step 0. It clones
    // --- the other repositories rather than depending on them.
